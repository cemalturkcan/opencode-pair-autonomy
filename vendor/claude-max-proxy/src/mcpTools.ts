...
/**
 * Resolve the effective working directory for MCP tools.
 * Priority:
 *   1. CLAUDE_PROXY_WORKDIR env var (explicit override)
 *   2. ~/.config/opencode/proxy-workdir file (written by shell wrapper per-session)
 *   3. process.cwd() fallback (works when proxy is run directly from project dir)
 */
const getCwd = (): string => {
  if (process.env.CLAUDE_PROXY_WORKDIR) return process.env.CLAUDE_PROXY_WORKDIR;
  try {
    const dir = fsSync.readFileSync(WORKDIR_FILE, "utf-8").trim();
      ...
      async (args) => {
        try {
          ...
          const content = await fs.readFile(
            filePath,
            (args.encoding || "utf-8") as BufferEncoding,
          );
          return {
      ...
      async (args) => {
        try {
          ...
          if (!content.includes(args.oldString)) {
            return {
              content: [
                { type: "text", text: `Error: oldString not found in file` },
              ],
              isError: true,
            };
          }
          ...
          return {
            content: [
              { type: "text", text: `Successfully edited ${args.path}` },
            ],
          };
        } catch (error) {
          return {
